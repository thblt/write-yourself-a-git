...
would seem logical to use a commit-like object to represent the
...
the Github mirror]].  The index is made of three parts:
...
in that index file. The rest of the file is a series of entries.  An entry begins with a set of fixed-length
data, followed by a variable-length name.  It begins with the usual
file metadata: creation and modification time, mode, owner and group
id, size, and an object hash of course) followed by a variable-length
name, terminated by a NULL byte (0x00), that's the path for that
file).  The code is quite straightforward:
...
#+BEGIN_SRC python :tangle libwyag.py
  class GitIndex (object):
      ...
      def __init__(self, file):
          ...
          assert(self.signature == b"DIRC") # Stands for "DirCache"
          self.version = int.from_bytes(header[4:8], "big")
          assert (2 == self.version)
          ...
          for i in range(0, count):
              ...
              flags = content[idx+60: idx+62]
              # TODO Actually read the flags, reject or process if extended_flag
...
the index tree.  This is just to show how to use the index, we'll put
