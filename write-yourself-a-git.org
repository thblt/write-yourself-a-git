...
section (=[core]=) and three fields:
 ...
 - =filemode = true=: disable tracking of file mode changes in the work tree.
...
that specify its type: =blob=, =commit=, =tag= or =tree=.  This header is
...
git hold.  They live in subdirectories of =.git/refs=, and are text
...
existing object.  After the tags is created, the name =v12.78.52= refers
...
The =git rev-parse= commands does a lot, but one if its use case is
...
the wyag program, libreary and/or test suite, please read the
following before you start working.
 ...
 - The source file for /both/ the article and the program is
   =write-yourself-a-git.org=.  It's format may seem unusual, but it's
...
#+begin_src emacs-lisp :exports results :results table
  (list
   ...
   `("On commit" ,(format "=%s= (%s)"
                       (car (magit-git-lines "describe" "--always"))
...
| Key              | Value                              |
|------------------+------------------------------------|
| Creation date    | Fri May 25 13:33:30 2018           |
| On commit        | =6071c08= (*dirty*)                    |
